...
def topk_router(probs: torch.Tensor, k: int):
    """
    probs: (B, S, E) expert probabilities per token position.
    Returns:
      topk_weights: (S, B, K) renormalized weights
      topk_indices: (S, B, K) expert indices
    """
    topk_weights, topk_indices = torch.topk(probs, k=k, dim=-1)  # along experts dim
...
def batch_aware_top_p_router(probs: torch.Tensor,
                             top_p: float,
                             per_step_total,
                             fraction_of_experts_to_use):
    """
    For each timestep:
        Add every token at this timestep into a max-heap where the priority is the probability it assigns to its most weighted expert
        num_experts_allocated = 0

        while num_experts_allocated <= max_experts_to_allocate_across_batch and len(max_heap) > 0:
            neediest_token = max_heap.pop() # this is the token that "needs" its next expert the most
            Assign an expert to this token
            If this token is "satisfied" (the cumulative probability of its allocated experts is > top_p):
                continue
            Else:
                # this token still wants more experts
                Add token back to the max_heap, with a new priority equal to the probability mass it assigns to the next highest expert
    
    return topk_weights, topk_indices
    """

    """
    Batch-aware top-p expert selection with a global budget.

    Args:
        probs: Tensor (S, B, E), probabilities over experts per token (already softmaxed).
        top_p: float in (0, 1], cumulative mass target per token.
        max_experts_to_allocate_across_batch: int, global cap on number of expert assignments
                                              across ALL tokens at this timestep.

    Returns:
        topk_weights: (S, B, E) renormalized weights (sum to 1 over kept experts per token; 0 if none kept)
        topk_indices: (S, B, E) expert ids where kept; -1 where not kept
    """
    assert probs.dim() == 3, "probs must be (B, S, E)"
    B, S, E = probs.shape
    device, dtype = probs.device, probs.dtype

    # Pre-sort once for all steps: descending per token
    ...
    for s in range(S):
        # State per token at step s
        cum = torch.zeros(B, device=device, dtype=dtype)                   # cumulative kept mass per token
        ptr = torch.zeros(B, device=device, dtype=torch.long)              # next candidate expert position per token
        keep_sorted = torch.zeros((B, E), device=device, dtype=torch.bool) # which sorted experts kept per token
        max_experts = round(per_step_total[s].item() * fraction_of_experts_to_use)

        # Build initial heap across tokens: priority = prob of next candidate (their top expert initially)
        # Python heap is min-heap → push (-priority, token_id)
        heap = []
        # Token has a next candidate if ptr < E and cum < top_p
        has_next = (ptr < E) & (cum < top_p)
        for b in torch.where(has_next)[0].tolist():
            gain = float(sorted_vals[b, s, 0].item())
            if gain > 0.0:
                heapq.heappush(heap, (-gain, b))

        allocations = 0
        # Greedy allocation within this timestep until budget or no candidates
        while allocations < max_experts and heap:
            neg_gain, b = heapq.heappop(heap)
            j = int(ptr[b].item())
            # Assign this expert to token b
            keep_sorted[b, j] = True
            cum[b] = cum[b] + sorted_vals[b, s, j]
            ptr[b] = j + 1
            allocations += 1

            # If token b still below top_p and has more experts, reinsert with next gain
            if (cum[b] < top_p) and (ptr[b] < E):
                next_gain = float(sorted_vals[b, s, ptr[b]].item())
                if next_gain > 0.0:
                    heapq.heappush(heap, (-next_gain, b))

        # Build normalized weights for this timestep
        kept_vals = torch.where(keep_sorted, sorted_vals[:, s, :], torch.zeros_like(sorted_vals[:, s, :]))  # (B, E)
        denom = kept_vals.sum(dim=-1, keepdim=True).clamp_min(1e-9)  # tokens with no experts will have denom≈1e-9
        # For tokens with zero experts, we want all-zero weights; zero-out those rows explicitly:
        zero_rows = (keep_sorted.sum(dim=-1) == 0)                    # (B,)
        kept_norm = kept_vals / denom
        kept_norm[zero_rows] = 0.0

        kept_idx = torch.where(keep_sorted, sorted_idx[:, s, :], torch.full_like(sorted_idx[:, s, :], -1))

        # Scatter back to original expert order for this timestep
        w = torch.zeros((B, E), device=device, dtype=dtype)
        # Only scatter where indices are valid
        valid = kept_idx >= 0
        if valid.any():
            w.scatter_(dim=-1, index=kept_idx.clamp_min(0), src=kept_norm)

        weights_out[:, s, :] = w
        # Place original ids where weight > 0, else -1
        indices_out[:, s, :] = torch.where(
            w > 0,
            torch.arange(E, device=device).expand(B, E),
            torch.full_like(sorted_idx[:, s, :], -1),
        )
...
class SwitchMLP(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        if self.use_switch:
            ...
            self.top_k = config.experts_topk
    ...
    def forward(self, hidden_states):
        ...
        if self.top_k and self.top_k > 0:
            topk_weights, topk_ind = topk_router(route, self.top_k)
        else:
            topk_weights, topk_ind = top_p_sampling_batched_all_sequence(route, self.top_p_threshold)
            active = topk_weights > 0
            per_step_total = active.sum(dim=(0, 2))
            # topk_weights, topk_ind = batch_aware_top_p_router(route, self.top_p_threshold, per_step_total, 0.85)
            topk_weights, topk_ind = batch_aware_top_p_router_vectorized(route, self.top_p_threshold, per_step_total, 0.85)

        
        
        # print("PRINTING PER STEP EXPERT COUNT")
        # print(per_step_total.shape)
        # print("__________")
        # print(per_step_total)
        # print("MAX NUMBER OF EXPERTS FOR ANY TIMESTEP")
        # print(per_step_total.max())
...
class MoEForCausalLM(MoEPreTrainedModel):
    def __init__(self, config, **kwargs):
        super().__init__(config)
        # wandb.init(project="MoE_VRAM", name="Batch size = 681", resume="allow")
